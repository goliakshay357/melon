...
export function expandDiagramCommand(text: string, args: string): string {
	...
	const directive = [
		...
		"You pick the visual type: if you're ~95% sure of the best fit, draw it; if genuinely torn between two types, ask ONE short question (name your best guess + the alternative) and wait. Read ~/.melon/agent/skills/diagram-design/SKILL.md (MELON CHAT MODE section) and the references/type-*.md for your chosen type.",
		...
		"- LABELS MUST FIT: mono text ≈ 0.6 × font-size × chars wide, sans ≈ 0.55 ×. A line wider than its box minus 16px padding is a failed render — shorten the label (drop list items) instead of overflowing. Tag chips sit fully inside the box. Sublabel lists: max 3 short items.",
