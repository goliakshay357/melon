...
import { formatEnvelopeWakeLines } from "./box-mail-envelope.ts";
...
export function sessionIsStreaming(registry: SessionRegistry, cardId: string): boolean {
...
export async function injectBoxMailToRecipient(args: {
	...
	mailId?: string;
	envelope?: BoxMailEnvelope;
}): Promise<{ inboundText: string; wake: string; display: string }> {
	const s = args.registry.get(args.toCardId);
	if (!s) throw Object.assign(new Error("recipient not attached"), { statusCode: 404 });
...
export async function deliverBoxMailToRecipient(args: {
	...
	mailId?: string;
	envelope?: BoxMailEnvelope;
}): Promise<{ delivery: "queued" | "ready"; wake: string; display: string; wakeQueued: boolean }> {
	const s = args.registry.get(args.toCardId);
	if (!s) throw Object.assign(new Error("recipient not attached"), { statusCode: 404 });
...
export async function recordBoxMailOutbound(args: {
	...
	if (!s) throw Object.assign(new Error("sender not attached"), { statusCode: 404 });
