...
type BoxMailBinding = {
	cardId: string;
	peers: BoxPeer[];
	broadcast: (payload: BoxMailIntentPayload) => void;
};

const byCardId = new Map<string, BoxMailBinding>();
/** sessionManager.getSessionId() → Melon card id (extensions only see the session). */
const cardIdBySessionId = new Map<string, string>();

export function bindBoxMailHost(
	cardId: string,
	broadcast: (payload: BoxMailIntentPayload) => void,
	sessionId?: string,
): void {
	...
	if (sessionId) cardIdBySessionId.set(sessionId, cardId);
}

export function unbindBoxMailHost(cardId: string): void {
	byCardId.delete(cardId);
	for (const [sessionId, id] of cardIdBySessionId) {
		if (id === cardId) cardIdBySessionId.delete(sessionId);
	}
}

export function cardIdForSessionId(sessionId: string | undefined): string | undefined {
	if (!sessionId) return undefined;
	return cardIdBySessionId.get(sessionId);
}

export function setBoxPeers(cardId: string, peers: BoxPeer[]): void {
...
export function getBoxPeers(cardId: string): BoxPeer[] {
...
export function formatBoxDirectory(peers: readonly BoxPeer[]): string {
	...
	for (const peer of peers.slice(0, 40)) {
		...
		if (peer.agentProfileId) bits.push(`profile: ${peer.agentProfileId}`);
		if (peer.agentInstanceName) bits.push(`instance: ${peer.agentInstanceName}`);
		if (peer.status) bits.push(`status: ${peer.status}`);
		lines.push(bits.join(" · "));
	}
	return lines.join("\n");
}

export function resolveBoxPeer(
	cardId: string,
	target: string,
): { peer?: BoxPeer; match: "card" | "profile" | "instance" | "title" | "none"; profileOnly?: string } {
	const raw = target.trim();
	if (!raw) return { match: "none" };
	const peers = getBoxPeers(cardId);
	const lower = raw.toLowerCase();

	const byId = peers.find((p) => p.cardId === raw);
	if (byId) return { peer: byId, match: "card" };

	const byInstance = peers.find((p) => p.agentInstanceName?.toLowerCase() === lower);
	if (byInstance) return { peer: byInstance, match: "instance" };

	const byProfileExact = peers.filter((p) => p.agentProfileId?.toLowerCase() === lower);
	if (byProfileExact.length === 1) return { peer: byProfileExact[0], match: "profile" };
	if (byProfileExact.length > 1) return { match: "none" }; // ambiguous — caller lists them

	const byTitle = peers.find((p) => p.title.toLowerCase().includes(lower));
	if (byTitle) return { peer: byTitle, match: "title" };
