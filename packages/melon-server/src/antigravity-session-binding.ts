...
function isAntigravityProvider(runtime: SessionRuntime): boolean {
	return (runtime.session.model?.provider ?? "").toLowerCase() === ANTIGRAVITY_PROVIDER_ID;
}

function antigravityIsolationUnavailableError(): Error & { statusCode: number } {
	return Object.assign(
		new Error(
			"Antigravity session isolation is unavailable. Melon requires the bundled pi-antigravity extension entry. Reinstall desktop dependencies and restart Melon.",
		),
		{ statusCode: 503 },
	);
}

function requireAntigravityIsolation(): void {
 ...
 */
export function stripAntigravitySessionEntriesFromSessionFile(sessionFile: string): number {
...
export async function activateAntigravitySessionBinding(
	runtime: SessionRuntime,
	options: ActivateAntigravitySessionBindingOptions = {},
): Promise<void> {
	if (!isAntigravityProvider(runtime)) return;
...
export async function runInBoundAntigravitySession<T>(
	runtime: SessionRuntime,
	options: ActivateAntigravitySessionBindingOptions,
	run: () => Promise<T>,
): Promise<T> {
	if (!isAntigravityProvider(runtime)) return run();
	requireAntigravityIsolation();
