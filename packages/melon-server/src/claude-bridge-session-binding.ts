...
function isClaudeBridgeProvider(runtime: SessionRuntime): boolean {
	return (runtime.session.model?.provider ?? "").toLowerCase() === CLAUDE_BRIDGE_PROVIDER_ID;
}

function claudeBridgeIsolationUnavailableError(): Error & { statusCode: number } {
	return Object.assign(
		new Error(
			"Claude Code session isolation is unavailable. Melon requires @fractaal/pi-claude-bridge's isolated entry (bundle/isolated.js). Reinstall desktop dependencies and restart Melon.",
		),
		{ statusCode: 503 },
	);
}

function requireClaudeBridgeIsolation(): void {
 ...
 * were removed.
 */
export function stripClaudeBridgeSessionEntriesFromSessionFile(sessionFile: string): number {
 ...
 */
export async function activateClaudeBridgeSessionBinding(
	runtime: SessionRuntime,
	options: ActivateClaudeBridgeSessionBindingOptions = {},
): Promise<void> {
	if (!isClaudeBridgeProvider(runtime)) return;
 ...
 */
export async function runInBoundClaudeBridgeSession<T>(
	runtime: SessionRuntime,
	options: ActivateClaudeBridgeSessionBindingOptions,
	run: () => Promise<T>,
): Promise<T> {
	if (!isClaudeBridgeProvider(runtime)) return run();
	requireClaudeBridgeIsolation();
