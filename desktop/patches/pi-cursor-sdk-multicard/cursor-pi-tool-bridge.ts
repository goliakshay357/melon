 ...
 * in a shared Node process. Bridges are keyed by extension runner (`pi`): a new
 ...
 */
const registeredCursorPiToolBridges = new Set<CursorPiToolBridgeRegistry>();
const bridgesBySessionScopeKey = new Map<string, CursorPiToolBridgeRegistry>();
const sessionScopeKeysByBridge = new Map<CursorPiToolBridgeRegistry, string>();
let activeCursorPiToolBridge: CursorPiToolBridgeRegistry | undefined;

interface CursorPiToolBridgeSessionContext {
	sessionManager?: { getSessionFile?: () => string | undefined; getSessionId?: () => string | undefined };
}

function sessionScopeKeyFromContext(ctx: CursorPiToolBridgeSessionContext | undefined): string {
...
function trackBridgeSessionScope(bridge: CursorPiToolBridgeRegistry, scopeKey: string): void {
...
function untrackBridgeSessionScope(bridge: CursorPiToolBridgeRegistry): void {
...
export function registerCursorPiToolBridge(pi: CursorPiToolBridgeExtensionApi): CursorPiToolBridge {
	...
	pi.on("session_shutdown", async (event) => {
		...
		bridgeToolExecutionAbortTracker.abortMatching(
			(toolCallId) => bridge.hasPendingPiToolCallId(toolCallId),
			reason,
		);
...
const warnedMissingSessionScopeKeys = new Set<string>();
...
export function getRegisteredCursorPiToolBridge(): CursorPiToolBridge | undefined {
	if (!isCursorHostSessionIsolationEnabled()) return activeCursorPiToolBridge;
	const hostScopeKey = getCursorHostSessionScopeKey();
	if (!hostScopeKey) return activeCursorPiToolBridge;
...
export const __testUtils = {
	...
	getBridgeForSessionScopeKeyForTests(scopeKey: string) {
