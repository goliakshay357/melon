...
interface CursorSessionScopeState {
	sessionCwd: string;
...
const state: CursorSessionScopeState = {
 ...
 */
const sessionStates = new Map<string, CursorSessionScopeState>();
...
function scopeKeyOf(scope: CursorSessionScopeState): string {
...
function readState(): CursorSessionScopeState {
	if (!isCursorHostSessionIsolationEnabled()) return state;
	const hostScopeKey = getCursorHostSessionScopeKey();
	if (!hostScopeKey) return state;
...
export function registerCursorSessionScope(pi: CursorSessionScopeExtensionApi): void {
	...
	pi.on("session_info_changed", (event, ctx) => {
		...
		if (scoped) scoped.sessionName = sessionName;
	});
