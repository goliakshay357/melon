...
import { readAgentProfile } from "./agents.js";
import { BOX_MAIL_REPLY_REASONS } from "./box-mail-envelope.js";
import { cardIdForSessionId, emitBoxMailIntent, getBoxPeers, resolveBoxPeer } from "./box-mail-host.js";
export const MELON_SEND_TO_BOX_TOOL_NAME = "send_to_box";
const SendToBoxParamsSchema = Type.Object({
    target: Type.String({
        description: "Who to message: another node's cardId, agent profile id, or instance short name (from the canvas nodes list in your system prompt).",
    }),
    message: Type.String({
        description: "What to send. Short, purposeful — like texting a teammate. No courtesy-only acks.",
    }),
    replyReason: Type.Optional(Type.String({
        description: `Only when replying to a node that mailed you: ${BOX_MAIL_REPLY_REASONS.join(" | ")}. Omit for a new A→B handoff.`,
    })),
    inReplyToMailId: Type.Optional(Type.String({
        description: "Inbound mailId from the wake when replying. Prefer this over guessing.",
    })),
});
function resolveFromCardId(ctx) {
    try {
...
function describePeers(cardId) {
    ...
    return peers
        ...
        if (p.agentProfileId)
            bits.push(`profile ${p.agentProfileId}`);
        if (p.agentInstanceName)
            bits.push(`instance ${p.agentInstanceName}`);
        if (p.status)
            bits.push(p.status);
        return `- ${bits.join(", ")}`;
    })
...
export default function melonSendToBoxExtension(pi) {
    pi.registerTool({
        name: MELON_SEND_TO_BOX_TOOL_NAME,
        label: "Send to node",
        description: "Send a message to ANOTHER Melon canvas node (async inbox, Approve by default). Pattern: hand off A→B once; optional single reply B→A with replyReason. Do not mail back and forth further. No courtesy acks.",
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            const envelope = {};
            if (replyReason)
                envelope.replyReason = replyReason;
            if (inReplyToMailId)
                envelope.parentMailId = inReplyToMailId;
            const resolved = resolveBoxPeer(fromCardId, target);
            if (resolved.peer) {
                return textResult(emitBoxMailIntent({
                    fromCardId,
            ...
            const ambiguous = getBoxPeers(fromCardId).filter((peer) => peer.agentProfileId?.toLowerCase() === target.toLowerCase());
            if (ambiguous.length > 1) {
                return textResult([
                    `Several nodes use profile "${target}" — pick one cardId:`,
                    ...ambiguous.map((peer) => `- ${peer.title} (cardId: ${peer.cardId}, instance: ${peer.agentInstanceName ?? "—"})`),
                ].join("\n"));
            }
...
//# sourceMappingURL=melon-send-to-box-extension.js.map