...
export function getNoteJob(jobId) {
    return jobs.get(jobId);
}
/** Emit to the buffered log and every live subscriber. */
export function emitNoteJob(job, event) {
 ...
 */
export function createDeltaPump(job) {
    let buffer = "";
    let timer = null;
    const flush = () => {
        if (timer) {
...
//# sourceMappingURL=note-jobs.js.map