...
function readMeta(id) {
    ...
    try {
        ...
        const defaultSkillIds = Array.isArray(raw.defaultSkillIds)
            ? raw.defaultSkillIds.filter((x) => typeof x === "string")
            : [];
        return {
            id,
            name,
...
export function topAgentProfiles(limit = 5) {
    return listAgentProfiles().slice(0, Math.max(0, limit));
}
export function readAgentProfile(id) {
...
export function createAgentProfile(input) {
    ...
    if (readMeta(id))
        throw Object.assign(new Error(`An agent named "${id}" already exists`), { statusCode: 409 });
...
export function updateAgentProfile(id, input) {
    const existing = readMeta(id);
    if (!existing)
        throw Object.assign(new Error(`unknown agent: ${id}`), { statusCode: 404 });
    const name = input.name.trim();
    const role = input.role.trim();
    if (!name)
        throw Object.assign(new Error("name is required"), { statusCode: 400 });
...
export function touchAgentProfileRecent(id) {
    const meta = readMeta(id);
    if (!meta)
        return;
    const ts = nowIso();
