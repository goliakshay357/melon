...
import { gitIn, isMelonWorktreePath, withProjectGitLock } from "./worktree.js";
const execFileAsync = promisify(execFile);
function classifyPorcelain(code) {
    if (code.includes("D"))
        return "removed";
    if (code.includes("A") || code === "??" || code.includes("?"))
        return "added";
    return "changed";
}
...
async function aheadBehind(worktreePath, baseBranch) {
...
async function tryGhPrCreate(worktreePath, baseBranch, branch, title, body) {
    try {
        const { stdout } = await execFileAsync("gh", ["pr", "create", "--base", baseBranch, "--head", branch, "--title", title, "--body", body], { cwd: worktreePath, timeout: 60_000, maxBuffer: 2 * 1024 * 1024 });
...
export async function shareCanvasWork(projectRoot, options) {
    if (!options.confirm) {
        return {
            ok: false,
            committed: false,
...
//# sourceMappingURL=canvas-share.js.map