...
## Melon embedding (when running inside Melon)

After a successful `deliver`, ALWAYS close your final message with a fenced
block whose body is ONE line — the delivered HTML file's absolute path, a
pipe (|), then the session working directory — so the diagram renders inline
in the chat card:

```
```viz-file
/abs/path/to/artifact.html|/abs/session/cwd
```
```

Never just link the artifact in prose — the fence is what embeds it.
