...
import { applyClaudeBridgeRuntimeEnv, getClaudeBridgeRuntimeStatus, requireClaudeBridgeRuntimeReady, } from "./claude-bridge-runtime.js";
...
function isIsolationProviderId(provider) {
...
function providerLabel(provider) {
    ...
    if (id === CURSOR_PROVIDER_ID)
        return "Cursor";
    if (id === CLAUDE_BRIDGE_PROVIDER_ID)
        return "Claude Code";
...
export async function buildApp(deps = {}) {
    ...
    const sessionAttachLocks = new Map();
    ...
    const manualWatchers = new Map(); // cwd -> FSWatcher
    const manualDebounces = new Map();
    function flushManualChanges(cwd) {
        const d = manualDebounces.get(cwd);
        if (!d || d.pending.size === 0)
            return;
    ...
    function ensureManualWatcher(cwd) {
        if (!cwd || manualWatchers.has(cwd))
            return;
        const dir = join(cwd, ".melon", "notes", "manual");
        try {
            if (!existsSync(dir))
            ...
            console.error(`[melon] manual watcher failed for ${dir}:`, err.message);
        }
    }
    async function withSessionAttachLocks(keys, run) {
        const releases = [];
        for (const key of [...new Set(keys)].sort()) {
            ...
            releases.push(() => {
                release();
                if (sessionAttachLocks.get(key) === queued)
                    sessionAttachLocks.delete(key);
            });
        }
        try {
            return await run();
    ...
    async function runInBoundIsolationSession(runtime, options, run) {
     ...
     */
    function broadcastInbox(cardId) {
     ...
     */
    async function flushNextApprovedInbox(cardId) {
        ...
        try {
            ...
            try {
                await runInBoundIsolationSession(s.runtime, { uiContext: s.extensionUi?.getUIContext() }, () => s.runtime.session.prompt(result.wake));
                console.log("[box-mail] wake prompt:end", { cardId });
            }
            catch (e) {
                console.error(`[box-mail] wake prompt THREW ${cardId}:`, e?.stack ?? e);
                ...
                if (isolationTurnId === undefined) {
                    ...
                    if (!s.draining)
                        s.busy = false;
                    if (!isIsolationTurnAborted(s, isolationTurnId))
                        afterAgentIdle(cardId);
                }
            }
        }
        catch (e) {
            console.error(`[box-mail] flush failed ${cardId}:`, e?.message ?? e);
        }
    }
    /** After the agent goes idle: drain user queue first, then approved inbox. */
    function afterAgentIdle(cardId) {
        const s = registry.get(cardId);
        if (!s || s.busy || s.draining)
            return;
        if (s.promptQueue.length > 0) {
            drainPromptQueue(cardId);
            return;
    ...
    function drainPromptQueue(cardId) {
        ...
        void run().finally(() => {
            ...
            if (lastIsolationTurnId === undefined || isCurrentIsolationTurn(s, lastIsolationTurnId))
                s.busy = false;
            afterAgentIdle(cardId);
        });
    }
    ...
    async function createRuntimeFor(sessionManager, enabledSkills = [], cardId, agentProfileId) {
        const factory = async ({ cwd, sessionManager: sm, sessionStartEvent, }) => {
            ...
            const appendSystemPromptOverride = (base) => {
                ...
                if (agentProfileId) {
                    ...
                    if (profile)
                        parts.push(formatAgentStandingInstructions(profile));
                }
                if (cardId)
                    parts.push(formatBoxDirectory(getBoxPeers(cardId)));
                return parts;
            };
    ...
    async function attachSession(cardId, sessionManager, explicitModel, skills = [], mode = "replace", explicitThinkingLevel, agentProfileId) {
        ...
        if (!needsIsolationLocks) {
            return attachSessionUnlocked(cardId, sessionManager, explicitModel, skills, mode, explicitThinkingLevel, agentProfileId);
        }
        const sessionFile = sessionManager.getSessionFile?.();
        return withSessionAttachLocks([`card:${cardId}`, ...(sessionFile ? [`session:${sessionFile}`] : [])], () => attachSessionUnlocked(cardId, sessionManager, explicitModel, skills, mode, explicitThinkingLevel, agentProfileId));
    }
    async function attachSessionUnlocked(cardId, sessionManager, explicitModel, skills = [], mode = "replace", explicitThinkingLevel, agentProfileId) {
        const wanted = explicitModel?.trim() || getDefaultModel(config.defaultModel);
        ...
        if (wantsAntigravity && !antigravitySessionIsolationAvailable()) {
            throw httpError(503, "Antigravity is unavailable because its bundled extension entry is missing. Reinstall desktop dependencies and restart Melon.");
        }
        ...
        if (wantsIsolation && incomingSessionFile) {
            for (const [ownerCardId, attached] of registry.entries()) {
                ...
                if (ownerProvider !== wantedProvider.toLowerCase())
                    continue;
        ...
        if (existing && (mode === "create" || existingIsIsolation || wantsIsolation)) {
            ...
            if (existing.busy) {
                try {
                    ...
                    console.error(`[${cardId}] ${providerLabel(existingProvider || wantedProvider)} runtime replacement abort failed:`, e.message);
                }
            }
        ...
        try {
            ...
            console.error(`[${cardId}] box-mail host bind failed:`, e?.message ?? e);
        }
        return runtime;
    }
    ...
    function resolveAgentProfileId(raw) {
    ...
    function mergeProfileSkills(skills, agentProfileId) {
        if (!agentProfileId)
            return skills;
        const profile = readAgentProfile(agentProfileId);
        if (!profile?.defaultSkillIds.length)
            return skills;
        return [...new Set([...profile.defaultSkillIds, ...skills])];
    }
    /** Idle specialized boxes re-read description.md after a Settings edit (FR-1.5). */
    async function refreshIdleProfileSessions(profileId) {
        for (const [cardId, s] of registry.entries()) {
            if (s.agentProfileId !== profileId || s.busy)
                continue;
            try {
                await s.runtime.session.reload?.();
            }
            catch (e) {
                console.error(`[${cardId}] agent profile reload failed:`, e?.message ?? e);
            }
        }
    }
    app.post("/sessions", async (req, reply) => {
        ...
        const skills = mergeProfileSkills(Array.isArray(body?.skills)
            ? body.skills.filter((x) => typeof x === "string")
            : [], agentProfileId);
        const runtime = await attachSession(cardId, SessionManager.create(dir), body?.model, skills, "create", typeof body?.thinkingLevel === "string" ? body.thinkingLevel : undefined, agentProfileId);
        if (agentProfileId)
            touchAgentProfileRecent(agentProfileId);
    ...
    app.post("/sessions/resume", async (req, reply) => {
        ...
        const runtime = await attachSession(cardId, SessionManager.open(sessionFile, undefined, cwdOverride), body?.model, skills, "resume", typeof body?.thinkingLevel === "string" ? body.thinkingLevel : undefined, agentProfileId);
        if (agentProfileId)
            touchAgentProfileRecent(agentProfileId);
        const resumedCwd = String(runtime.session.sessionManager.getCwd?.() ?? "");
        if (resumedCwd)
            ensureManualWatcher(resumedCwd);
        return {
            cardId,
            sessionId: runtime.session.sessionId,
            sessionFile,
    ...
    app.post("/sessions/:cardId/fork", async (req, reply) => {
        ...
        if (strippedCursor > 0) {
            console.log(`[melon] fork ${parentCardId}→${newCardId}: stripped ${strippedCursor} cursor resume handle(s) from child session`);
        }
        const strippedClaude = stripClaudeBridgeSessionEntriesFromSessionFile(childSessionFile);
        if (strippedClaude > 0) {
            console.log(`[melon] fork ${parentCardId}→${newCardId}: stripped ${strippedClaude} claude-bridge session marker(s) from child session`);
        }
        const strippedAntigravity = stripAntigravitySessionEntriesFromSessionFile(childSessionFile);
        if (strippedAntigravity > 0) {
            console.log(`[melon] fork ${parentCardId}→${newCardId}: stripped ${strippedAntigravity} antigravity session marker(s) from child session`);
        }
    ...
    app.post("/sessions/:cardId/tree", async (req, reply) => {
        ...
        if (!entryId)
            return reply.code(400).send({ error: "entryId required" });
        ...
        if (s.busy)
            return reply.code(409).send({ error: "card is streaming" });
        const sessionFile = s.runtime.session.sessionFile;
        ...
        try {
            ...
            if (result.cancelled)
                return reply.code(409).send({ error: "navigation cancelled" });
            ...
            console.log(`[${cardId}] tree:navigate entry=${entryId} live=${wasLive} -> leaf=${s.runtime.session.sessionManager.getLeafId()}`);
            ...
            console.error(`[${cardId}] tree:navigate failed`, e.stack);
            return reply.code(400).send({ error: e.message });
        }
    });
    ...
    app.post("/canvases/:id/autoname", async (req, reply) => {
        ...
        const card = (canvas.cards ?? []).find((c) => typeof c?.sessionFile === "string");
        if (!card?.sessionFile)
            return { skipped: true, reason: "no chat yet" };
        let firstUser = "";
        let firstAssistant = "";
        try {
            ...
            for (const e of ctx) {
                ...
                if (m.role === "user" && !firstUser)
                    firstUser = textOf(m.content).trim();
        ...
        if (!firstUser)
            return { skipped: true, reason: "no user message" };
        let name = "";
        try {
            ...
            const res = await mr.completeSimple(model, {
                systemPrompt: "You name chat workspaces. Reply with ONLY a short title of 2 to 5 words, Title Case. No quotes, no ending punctuation, no explanation.",
                messages: [
                    {
                        role: "user",
                        content: [
    ...
    app.post("/notes/manual/rename", async (req, reply) => {
        ...
        if (join(dirAbs, name) !== current)
            renameSync(current, join(dirAbs, name));
    ...
    app.post("/notes/manual/delete", async (req, reply) => {
        ...
        try {
            ...
            return reply.code(500).send({ error: `could not delete manual: ${e.message}` });
        }
    ...
    app.get("/files", async (req, reply) => {
        ...
        console.log(`[files] q="${String(q.q ?? "")}" cwd=${dir} -> ${out.length} hits (${Date.now() - filesT0}ms) top: ${out
            .slice(0, 5)
    ...
    app.post("/files/exists", async (req, reply) => {
        ...
        const paths = Array.isArray(body?.paths)
            ? body.paths.filter((x) => typeof x === "string").slice(0, 100)
            : [];
    ...
    async function completeNoteText(systemPrompt, prompt, modelString) {
        const [provider, modelId] = splitModel(modelString);
        const model = (await getModelRuntime()).getModel(provider, modelId);
        if (!model)
            throw httpError(400, `unknown model: ${modelString}`);
        try {
            const response = await (await getModelRuntime()).complete(model, {
                systemPrompt,
    ...
    function resolveNoteSource(body) {
        ...
        if (typeof body?.sessionFile === "string" && body.sessionFile.trim()) {
            ...
            if (!existsSync(file))
                throw httpError(400, `session file not found: ${file}`);
            const sm = SessionManager.open(file);
            const leafId = typeof body?.leafEntryId === "string" &&
    ...
    function wantedModelFor(body, sourceCardId) {
    ...
    app.post("/notes/generate", async (req, reply) => {
        ...
        if (!conversationText.trim())
            return reply.code(422).send({ error: "no conversation to distill" });
        const wantedModel = wantedModelFor(body, body?.sourceCardId);
        let generated;
        try {
            generated = await completeNoteText(HANDOFF_ARTIFACT_SYSTEM_PROMPT, `## Session transcript\n\n${conversationText}`, wantedModel);
        }
        catch (e) {
        ...
        const doc = {
            id: newNoteId(),
            kind: "handoff",
            title: String(body?.sourceTitle ?? "Handoff")
                .trim()
                .slice(0, 80) || "Handoff",
            revision: 1,
            created: now,
    ...
    app.get("/notes", async (req) => {
        const dir = expandHome(req.query?.cwd ?? "");
    ...
    app.post("/notes/:id/wire", async (req, reply) => {
        ...
        if (!targetCardId)
            return reply.code(400).send({ error: "targetCardId required" });
        ...
        if (existing) {
            ...
            runtime = await attachSession(targetCardId, sessionFile ? SessionManager.open(sessionFile) : SessionManager.create(sessionCwd), body?.model, skills, "create", typeof body?.thinkingLevel === "string" ? body.thinkingLevel : undefined);
        }
        const envelope = `[Context handoff from card "${doc.sources[0]?.cardTitle ?? doc.title}" — reference material for future turns, not a request.\nArtifact ${doc.id} r${doc.revision} — file: ${doc.filePath ?? notePath(dir, doc.id)}]`;
        const messageText = `${envelope}\n\n${doc.body}`;
        const hasMessages = runtime.session.sessionManager.getEntries().some((e) => e.type === "message");
        let mode;
        if (hasMessages) {
    ...
    app.post("/notes/merge/generate", async (req, reply) => {
        ...
        if (rawSources.length < 2)
            return reply.code(400).send({ error: "at least two sources required" });
        ...
        try {
            for (const src of rawSources) {
                ...
                const cardTitle = src.title ?? (src.cardId ? body?.sourceTitles?.[src.cardId] : undefined) ?? src.cardId ?? "source";
                const text = ensureNotesMine(await completeNoteText(HANDOFF_ARTIFACT_SYSTEM_PROMPT, `## Session transcript\n\n${conversationText}`, wantedModelFor(body, src.cardId)));
                const now = new Date().toISOString();
                const perSource = {
                    id: newNoteId(),
                    kind: "handoff",
        ...
        let merged;
        try {
            merged = ensureNotesMine(await completeNoteText(MERGE_ARTIFACT_SYSTEM_PROMPT, `## Source handoff artifacts\n\n${sourcesBlock}`, wantedModel));
        }
        catch (e) {
            return reply.code(e.statusCode ?? 502).send({ error: e.message });
        }
        ...
        const doc = {
            id: newNoteId(),
            kind: "merge",
            title: String(body?.title ?? `Merge of ${distillations.length}`)
                .trim()
                .slice(0, 80) || "Merge",
            revision: 1,
            created: now,
    ...
    app.post("/notes/:id/regenerate", async (req, reply) => {
        ...
        try {
            if (doc.kind === "merge") {
                ...
                for (const s of doc.sources) {
                    ...
                    if (!text) {
                        ...
                        const conversationText = serializeBranchForHandoff(resolved.sm.getBranch(resolved.leafId));
                        if (!conversationText.trim())
                            throw httpError(422, `source session has no conversation`);
                        text = ensureNotesMine(await completeNoteText(HANDOFF_ARTIFACT_SYSTEM_PROMPT, `## Session transcript\n\n${conversationText}`, wantedModel));
                    }
                    parts.push(`## Source: "${title}" (artifact ${s.handoffId ?? "n/a"})\n\n${text}`);
                }
                const proposal = ensureNotesMine(await completeNoteText(MERGE_ARTIFACT_SYSTEM_PROMPT, `## Source handoff artifacts\n\n${parts.join("\n\n---\n\n")}`, wantedModel));
                return { ok: true, proposal };
            }
            const src = doc.sources[0];
            if (!src?.sessionFile)
                return reply.code(422).send({ error: "artifact has no session provenance" });
            const resolved = resolveNoteSource({
            ...
            if (!conversationText.trim())
                return reply.code(422).send({ error: "no conversation to distill" });
            const proposal = ensureNotesMine(await completeNoteText(HANDOFF_ARTIFACT_SYSTEM_PROMPT, `## Session transcript\n\n${conversationText}`, wantedModel));
            return { ok: true, proposal };
        }
        catch (e) {
            return reply.code(e.statusCode ?? 502).send({ error: e.message });
    ...
    app.post("/notes/:id/refine", async (req, reply) => {
        ...
        try {
            ...
            const proposal = ensureNotesMine(await completeNoteText(REFINE_ARTIFACT_SYSTEM_PROMPT, `## Current artifact\n\n${doc.body}${grounding}\n\n## Instruction\n\n${instruction}`, wantedModel));
            return { ok: true, proposal };
        }
        catch (e) {
            return reply.code(e.statusCode ?? 502).send({ error: e.message });
    ...
    const HANDOFF_SKELETON = "## Goal\n\n## Findings\n\n## Decisions\n\n## Gotchas\n\n## Open questions\n\n## Evidence (do not re-derive)\n\n## Handoff notes (mine)\n";
    const MERGE_SKELETON = "## Shared goal\n\n## Decisions\n\n## Conflicts to resolve\n\n## Open questions\n\n## Evidence (do not re-derive)\n\n## Next steps\n\n## Handoff notes (mine)\n";
    /** One streaming LLM call; deltas are pumped into the job's event log. */
    async function streamNoteText(job, systemPrompt, prompt, modelString) {
        const [provider, modelId] = splitModel(modelString);
        const model = (await getModelRuntime()).getModel(provider, modelId);
        if (!model)
            throw httpError(400, `unknown model: ${modelString}`);
        ...
        try {
            const stream = (await getModelRuntime()).stream(model, {
                systemPrompt,
                messages: [{ role: "user", content: [{ type: "text", text: prompt }], timestamp: Date.now() }],
            }, { cacheRetention: "none", sessionId: uuidv7() });
            void (async () => {
    ...
    app.post("/notes/generate/start", async (req, reply) => {
        ...
        if (typeof body?.artifactId === "string" && body.artifactId) {
            if (!isValidNoteId(body.artifactId))
                return reply.code(400).send({ error: "invalid note id" });
        ...
        void (async () => {
            try {
                ...
                if (!sessionFile || !existsSync(sessionFile))
                    throw httpError(422, "source session is gone");
                ...
                if (!conversationText.trim())
                    throw httpError(422, "no conversation to distill");
                emitNoteJob(job, {
                    type: "status",
                ...
                const focusBlock = focus
                    ...
                    : "";
                const text = ensureNotesMine(await streamNoteText(job, HANDOFF_ARTIFACT_SYSTEM_PROMPT, `## Session transcript\n\n${conversationText}${focusBlock}`, model));
                ...
                emitNoteJob(job, { type: "error", message: e.message });
            }
        })();
        return {
            id: doc.id,
    ...
    app.get("/notes/jobs/:jobId/events", (req, reply) => {
        const job = getNoteJob(req.params.jobId);
        if (!job)
    ...
    app.post("/notes/merge/start", async (req, reply) => {
        ...
        if (rawSources.length < 2)
            return reply.code(400).send({ error: "at least two sources required" });
        const title = String(body?.title ?? "Merge")
            .trim()
            .slice(0, 80) || "Merge";
        const now = new Date().toISOString();
        const doc = {
        ...
        void (async () => {
            try {
                ...
                for (const src of rawSources) {
                    ...
                    const conversationText = serializeBranchForHandoff(resolved.sm.getBranch(resolved.leafId));
                    if (!conversationText.trim())
                        throw httpError(422, "a source has no conversation to distill");
                    const cardTitle = src.title ?? (src.cardId ? body?.sourceTitles?.[src.cardId] : undefined) ?? src.cardId ?? "source";
                    const text = ensureNotesMine(await streamNoteText(job, HANDOFF_ARTIFACT_SYSTEM_PROMPT, `## Session transcript\n\n${conversationText}`, model));
                ...
                const sourcesBlock = distillations
                    .map((d) => `## Source: "${d.title}" (artifact ${d.source.handoffId})\n\n${d.body}`)
                    .join("\n\n---\n\n");
                const merged = ensureNotesMine(await streamNoteText(job, MERGE_ARTIFACT_SYSTEM_PROMPT, `## Source handoff artifacts\n\n${sourcesBlock}`, model));
                emitNoteJob(job, { type: "status", message: "writing artifact" });
                runDoc.body = merged;
                runDoc.updated = new Date().toISOString();
                runDoc.edited = false;
    ...
    app.post("/notes/:id/restore", async (req, reply) => {
        ...
        if (!isValidNoteId(id))
            return reply.code(400).send({ error: "invalid note id" });
        if (!restoreNote(dir, id))
            return reply.code(404).send({ error: "not in trash" });
        const doc = loadNote(dir, id);
    ...
    app.post("/notes/:id/reveal", async (req, reply) => {
        ...
        const cmd = process.platform === "darwin"
            ...
            : process.platform === "win32"
                ...
                : ["xdg-open", dir];
        try {
            await new Promise((res, rej) => execFile(cmd[0], cmd.slice(1), { timeout: 15_000 }, (err) => (err ? rej(err) : res())));
        }
        catch (e) {
    ...
    app.get("/models", async (req) => {
        ...
        const error = providerKey === CURSOR_PROVIDER_ID && models.length === 0
    ...
    app.get("/agents/:id", async (req, reply) => {
        ...
        if (!agent)
            return reply.code(404).send({ error: `unknown agent: ${id}` });
        return agent;
    });
    app.post("/agents", async (req, reply) => {
        ...
        const defaultSkillIds = Array.isArray(b?.defaultSkillIds)
            ? b.defaultSkillIds.filter((x) => typeof x === "string")
            : [];
        if (!id || !name)
    ...
    app.post("/boxes/mail/send", async (req, reply) => {
        ...
        if (!mailBody)
            return reply.code(400).send({ error: "body required" });
        if (fromCardId === toCardId)
            return reply.code(400).send({ error: "cannot mail a node to itself" });
        ...
        const envelopeInput = body?.envelope && typeof body.envelope === "object" && !Array.isArray(body.envelope)
            ? { ...body.envelope }
            : {};
        if (body?.replyPolicy !== undefined)
            envelopeInput.replyPolicy = body.replyPolicy;
        if (body?.replyReason !== undefined)
            envelopeInput.replyReason = body.replyReason;
        if (body?.threadId !== undefined)
            envelopeInput.threadId = body.threadId;
        if (body?.parentMailId !== undefined)
            envelopeInput.parentMailId = body.parentMailId;
        const inReplyToMailId = typeof body?.inReplyToMailId === "string" ? body.inReplyToMailId : undefined;
        const resolveMailCwd = () => {
            const attachedFrom = registry.get(fromCardId);
            return assertCwd((typeof body?.cwd === "string" && body.cwd.trim()) ||
        ...
        const ensureMailCardAttached = async (args) => {
            ...
            const skills = mergeProfileSkills(Array.isArray(args.skills) ? args.skills.filter((x) => typeof x === "string") : [], agentProfileId);
            ...
            await attachSession(args.cardId, sessionFile ? SessionManager.open(sessionFile, undefined, dir) : SessionManager.create(dir), args.model, skills, sessionFile ? "resume" : "create", undefined, agentProfileId);
            if (!registry.get(args.cardId)) {
        ...
        try {
            await ensureMailCardAttached({
                ...
                model: (typeof body?.fromModel === "string" && body.fromModel) ||
            ...
            await ensureMailCardAttached({
                ...
                model: (typeof body?.toModel === "string" && body.toModel) ||
                    (typeof body?.model === "string" ? body.model : undefined),
        ...
        if (!from || !to)
            return reply.code(500).send({ error: "mail cards not attached" });
        try {
    ...
    app.post("/sessions/:cardId/inbox/:mailId/approve", async (req, reply) => {
        ...
        if (!registry.get(cardId)) {
            try {
                ...
                const agentProfileId = resolveAgentProfileId(body.agentProfileId);
                const skills = mergeProfileSkills(Array.isArray(body.skills) ? body.skills.filter((x) => typeof x === "string") : [], agentProfileId);
                ...
                await attachSession(cardId, sessionFile ? SessionManager.open(sessionFile, undefined, dir) : SessionManager.create(dir), typeof body.model === "string" ? body.model : undefined, skills, sessionFile ? "resume" : "create", undefined, agentProfileId);
            }
            catch (e) {
                const err = e;
        ...
        return {
            ...
            injectedText: after?.status === "delivered" || after?.status === "approved"
                ? boxMailInboundText({
    ...
    app.put("/settings", async (req, reply) => {
        ...
        if ("favoriteModels" in body) {
            ...
            next.favoriteModels = [...new Set(raw.map((m) => m.trim()))].slice(0, 50);
        }
        saveSettings(next);
    ...
    app.get("/auth/providers", async () => {
        ...
        for (const pid of [...allProviderIds].sort()) {
            ...
            const configured = pid === CURSOR_PROVIDER_ID
                ...
                : pid === CLAUDE_BRIDGE_PROVIDER_ID
                    ...
                    : pid === ANTIGRAVITY_PROVIDER_ID
                        ...
                        : !!status.configured;
            const error = pid === CURSOR_PROVIDER_ID && (!cursorStatus.loaded || cursorStatus.issues.length > 0)
            ...
            const sourceLabel = !configured
                ...
                : stored
                    ...
                    : status.source === "environment"
                        ? (status.label ?? "Environment variable")
    ...
    app.post("/sessions/:cardId/box-peers", async (req, reply) => {
        ...
        if (!Array.isArray(body?.peers))
            return reply.code(400).send({ error: "peers array required" });
        const peers = [];
    ...
    app.delete("/sessions/:cardId", async (req, reply) => {
        ...
        if (s.busy) {
            try {
                ...
                console.error(`[${cardId}] isolation-sensitive teardown abort failed:`, e.message);
            }
        }
        await s.runtime.dispose();
        unbindBoxMailHost(cardId);
