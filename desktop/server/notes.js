...
export function createManual(cwd, title) {
...
export function listManuals(cwd) {
    const dir = manualsDir(cwd);
    let files;
    try {
...
export function notePath(cwd, id) {
...
export function wireIsStale(doc, wire) {
...
export function snapshotRevision(cwd, doc) {
...
function emitMapEntries(lines, entries, indent) {
        ...
        if (Array.isArray(value)) {
            if (value.length === 0) {
                ...
                for (const item of value) {
                    ...
                    const mapEntries = Object.entries(item).filter(([, v]) => v !== undefined);
                    if (mapEntries.length === 0)
                        continue;
                    const [firstKey, firstValue] = mapEntries[0];
                    lines.push(`${indent}  - ${firstKey}: ${emitScalar(firstValue)}`);
                    for (const [k, v] of mapEntries.slice(1))
                        lines.push(`${indent}    ${k}: ${emitScalar(v)}`);
                }
            }
        }
        else if (typeof value === "object" && value !== null) {
            lines.push(`${indent}${key}:`);
            emitMapEntries(lines, Object.entries(value), `${indent}  `);
        }
        else {
...
function parseMapLine(text) {
    const sep = text.indexOf(": ");
    if (sep >= 0)
        return { key: text.slice(0, sep).trim(), value: parseScalar(text.slice(sep + 2)) };
    if (text.endsWith(":"))
        return { key: text.slice(0, -1).trim(), value: undefined };
...
export function parseNote(text) {
    ...
    if (parsed.next < lines.length)
        throw new Error("malformed frontmatter structure");
    const fm = parsed.value;
    if (!fm || typeof fm !== "object")
        throw new Error("frontmatter is not a mapping");
    const id = parseScalar(String(fm.id ?? ""));
    if (!isValidNoteId(id))
        throw new Error(`invalid note id: ${fm.id}`);
    const kindRaw = parseScalar(String(fm.kind ?? "handoff"));
    if (kindRaw !== "handoff" && kindRaw !== "merge")
        throw new Error(`unsupported note kind: ${kindRaw}`);
    ...
    return {
        ...
        history: Array.isArray(fm.history) ? fm.history.map((h) => parseScalar(String(h))) : [],
        wires,
        body: bodyLines.join("\n"),
        mtimeMs: 0,
    };
 ...
 */
export function resolveNoteFile(cwd, id) {
    ...
    for (const f of files) {
        ...
        const abs = join(dir, f);
        try {
            if (parseNote(readFileSync(abs, "utf8")).id === id)
                return abs;
        }
        catch {
            /* skip corrupt */
...
export function uniqueHandoffFileName(cwd, title, exceptAbs) {
    const dir = notesDir(cwd);
    const base = slugifyTitle(title);
    let name = `${base}.md`;
    let n = 2;
...
export function loadNote(cwd, id) {
...
export function saveNote(cwd, doc) {
 ...
 */
export function renameNoteFile(cwd, doc, newTitle) {
...
export function trashNote(cwd, id) {
    const path = resolveNoteFile(cwd, id);
    if (!path)
        return false;
    const trashDir = join(notesDir(cwd), ".trash");
...
function resolveTrashedNoteFile(cwd, id) {
    const trashDir = join(notesDir(cwd), ".trash");
    let files;
    try {
...
export function restoreNote(cwd, id) {
...
export function listTrashedNotes(cwd) {
    const trashDir = join(notesDir(cwd), ".trash");
    let files;
    try {
        files = readdirSync(trashDir);
